...
do
	...
	function library.new(title)
		local container = utility:Create("ScreenGui", {
			...
			utility:Create("ImageLabel", {
				...
				Position = UDim2.new(0.5,-512/2,0.5,-428/2),
				Size = UDim2.new(0, 512, 0, 428),
    ...
    function section:Button(buttonInfo)
        buttonInfo.text = tostring(buttonInfo.text)
        buttonInfo.callback = typeof(buttonInfo.callback) == 'function' and buttonInfo.callback or function() end
    ...
    function section:Toggle(toggleInfo)
        toggleInfo.text = tostring(toggleInfo.text)
        toggleInfo.state = typeof(toggleInfo.state) == 'boolean' and toggleInfo.state or false
        toggleInfo.callback = typeof(toggleInfo.callback) == 'function' and toggleInfo.callback or function() end
		...
		toggle.MouseButton1Click:Connect(function()
            ...
            toggleInfo.callback(active, function(...)
    ...
    function section:Textbox(boxInfo)
        boxInfo.text = tostring(boxInfo.text)
        boxInfo.value = typeof(boxInfo.value) == 'string' and boxInfo.value or ''
        boxInfo.callback = typeof(boxInfo.callback) == 'function' and boxInfo.callback or function() end
		...
		input.FocusLost:Connect(function()
            ...
            boxInfo.callback(input.Text, true, function(...)
    ...
    function section:Bind(bindInfo)
        bindInfo.text = tostring(bindInfo.text)
        bindInfo.key = typeof(bindInfo.key) == 'EnumItem' and bindInfo.key or nil
        bindInfo.callback = typeof(bindInfo.callback) == 'function' and bindInfo.callback or function () end
        bindInfo.newcallback = typeof(bindInfo.newcallback) == 'function' and bindInfo.newcallback or function() end
		...
		keybind.MouseButton1Click:Connect(function()
			...
			if text.Text == "None" then -- new bind
                ...
                bindInfo.newcallback(key, function(...)
                    self:updateKeybind(keybind, ...)
                end)
			end
		end)
    ...
    function section:ColorPicker(pickerInfo)
        pickerInfo.text = tostring(pickerInfo.text)
        pickerInfo.color = typeof(pickerInfo.color) == 'table' and Color3.fromRGB(pickerInfo.color[1],pickerInfo.color[2],pickerInfo.color[3]) or typeof(pickerInfo.color) == 'Color3' and pickerInfo.color or Color3.new(1,1,1)
        pickerInfo.callback = typeof(pickerInfo.callback) == 'function' and pickerInfo.callback or function() end
        ...
        canvas.MouseButton1Down:Connect(function()
            canvasSize = canvas.AbsoluteSize --broken shit fixed with 2 lines /shrug
            canvasPosition = canvas.AbsolutePosition
            draggingCanvas = true
        ...
        color.MouseButton1Down:Connect(function()
            colorPosition = color.AbsolutePosition
            colorSize = color.AbsoluteSize 
			draggingColor = true
    ...
    function section:Slider(sliderInfo)
        sliderInfo.text = tostring(sliderInfo.text)
        sliderInfo.min = typeof(sliderInfo.min) == 'number' and sliderInfo.min or 0
        sliderInfo.max = typeof(sliderInfo.max) == 'number' and sliderInfo.max or 100
        sliderInfo.float = 1
        sliderInfo.value = typeof(sliderInfo.value) == 'number' and sliderInfo.value or sliderInfo.min
        sliderInfo.editable = typeof(sliderInfo.editable) == 'boolean' and sliderInfo.editable or true
		...
		local slider = utility:Create("ImageButton", {
			...
			utility:Create("TextBox", {
                ...
                TextEditable = sliderInfo.editable,
				TextColor3 = themes.TextColor,
				TextSize = 12,
		...
		textbox.FocusLost:Connect(function()
			if not tonumber(textbox.Text) then
				value = self:updateSlider(slider, nil, sliderInfo.value, sliderInfo.min, sliderInfo.max)
				sliderInfo.callback(value)
			end
		end)
		...
		textbox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			elseif not allowed[text] then	
				value = self:updateSlider(slider, nil, tonumber(text) or value, sliderInfo.min, sliderInfo.max)
				sliderInfo.callback(value)
			end
		end)
    ...
    function section:Dropdown(dropdownInfo)
        dropdownInfo.text = tostring(dropdownInfo.text) .. (dropdownInfo.value and (': '.. dropdownInfo.value) or '')
        dropdownInfo.values = typeof(dropdownInfo.values) == 'table' and dropdownInfo.values or {}
        dropdownInfo.callback = typeof(dropdownInfo.callback) == 'function' and dropdownInfo.callback or function() end
		...
		search.Button.MouseButton1Click:Connect(function()
			if search.Button.Rotation == 0 then
				self:updateDropdown(dropdown, nil, dropdownInfo.values, dropdownInfo.callback)
			else
				self:updateDropdown(dropdown, nil, nil, callback)
			end
		end)
		
		search.TextButton.MouseButton1Down:Connect(function()
			if search.Button.Rotation == 0 then
				self:updateDropdown(dropdown, nil, dropdownInfo.values, dropdownInfo.callback)
			end			
		end)
	...
	function section:updateDropdown(dropdown, title, list, callback)
		...
		if title then
			dropdown.Search.TextButton.Text = title
		end
